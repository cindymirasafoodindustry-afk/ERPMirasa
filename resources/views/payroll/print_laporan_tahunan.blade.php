...
<head>
    ...
    <title>Cetak Analisis Buku Besar Payroll Tahunan — TAHUN {{ $tahun }}</title>
    <style>
        /* ========================================================================= */
        /* KUNCI SAKTI: PORTRAIT A4 ABADI (WEB VIEW & PRINT VIEW WAJIB KEMBAR TEGAK)  */
        /* ========================================================================= */
        @page { 
            size: A4 portrait; 
        ...
        body { 
            ...
            font-size: 10px; 
            color: #1e293b; 
            background-color: #cbd5e1; /* Latar luar kertas abu-abu gelap agar kontras kertas Portrait menonjol */
            line-height: 1.4; 
            margin: 0;
            padding: 40px 0;
        }

        /* Paksa Web View membentuk lembaran kertas Portrait A4 ramping di tengah monitor */
        .page-container {
            background-color: #ffffff;
            width: 210mm; /* Ukuran lebar mutlak kertas Portrait A4 */
            min-height: 297mm; /* Ukuran tinggi mutlak kertas Portrait A4 */
...
<body>
    ...
    <!-- 🚀 BLOK KALKULASI SAKRAL ATAS SINKRONISASI VARIABEL GRAND TOTAL GAJI -->
    @php
        // Langsung menghitung total jumlahan matematika dari array data harian
    ...
    <div class="page-container">
        ...
        <div class="header-container">
            <div class="header-left">
                ...
                <p>Laporan Rekapitulasi Analisis Biaya Gaji Operasional Tahunan Pabrik</p>
            </div>
        ...
        <div class="meta-container">
            <div class="meta-box">
                <span>PERIODE LAPORAN TAHUNAN</span>
                <strong>TAHUN ANGGARAN {{ $tahun }}</strong>
            </div>
        ...
        <div class="stats-grid">
            <div class="stat-card blue">
                <div class="stat-label">Total Anggaran Pengeluaran</div>
                <div class="stat-value">Rp {{ number_format($grandTotalGaji, 0, ',', '.') }}</div>
            </div>
            <div class="stat-card orange">
                <div class="stat-label">Total Kelompok Langsung</div>
                <div class="stat-value">Rp {{ number_format($grandTotalLangsung, 0, ',', '.') }}</div>
            </div>
            <div class="stat-card gray">
                <div class="stat-label">Kelompok Tdk Langsung</div>
                <div class="stat-value">Rp {{ number_format($grandTotalTidakLangsung, 0, ',', '.') }}</div>
            </div>
        </div>

        <div class="section-title">I. Rincian Akumulasi Buku Besar Gaji Harian Massal (Januari - Desember)</div>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width: 90px;">Karyawan Masuk</th>
                    <th style="width: 125px;">Kelompok Langsung (HPP)</th>
                    <th style="width: 135px;">Kelompok Tdk Langsung</th>
                    <th style="width: 115px;">Total Gaji Keseluruhan</th>
                </tr>
            </thead>
            <tbody>
                    @forelse($dailyReports as $index => $report)
                        <tr>
                            ...
                            <td>{{ $index }}</td>
                            <td>{{ $report['total_hadir'] }} Orang Masuk</td>
                            <td>Rp {{ number_format($report['gaji_langsung'], 0, ',', '.') }}</td>
                            <td>Rp {{ number_format($report['gaji_tidak_langsung'], 0, ',', '.') }}</td>
                            <td>Rp {{ number_format($report['total_gaji'], 0, ',', '.') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="text-center">Belum ada data jurnal absensi tahunan.</td>
                        </tr>
                    @endforelse
                    ...
                    @if(count($dailyReports) > 0)
                        <tr class="total-row">
                            ...
                            <td class="text-center">{{ $grandTotalHadir }} Orang Masuk</td>
